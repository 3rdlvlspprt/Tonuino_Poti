...
static void nextTrack(uint16_t track) {
  ...
  if (myFolder->mode == 1 || myFolder->mode == 7) {
    Serial.println(F("H�rspielmodus ist aktiv -> keinen neuen Track spielen"));
    setstandbyTimer();
...
static void previousTrack() {
  ...
  /*  if (myCard.mode == 1 || myCard.mode == 7) {
      Serial.println(F("H�rspielmodus ist aktiv -> Track von vorne spielen"));
...
void Checkvolume() {
  uint8_t nVol = map(analogRead(volPin), 0, 1024, mySettings.minVolume, mySettings.maxVolume);
  if ( abs(nVol - volume)>1) {
     volume = nVol;
